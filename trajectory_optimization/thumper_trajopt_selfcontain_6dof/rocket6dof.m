...
dyn_size = size(x,2);
if length(phase) ~= dyn_size
    phase = phase*ones(1,dyn_size);
end
...
pres = atmo_profile.P/param.scales.pressure;
...
Facsb = param.T_acs*param.acs_point*onoff;
mfueldot = vecnorm(Facsb)./param.Isp_acs./param.g0;
Facs = zeros(size(Fg));
...
% % 1st stage % %
% thrust
Ftb(:,ind_phase1) = throtl(:,ind_phase1).*param.maxT_1st.*tvc_body(:,ind_phase1);
mdot(:,ind_phase1) = vecnorm(Ftb(:,ind_phase1))./param.Isp1./param.g0 + mfueldot(:,ind_phase1);
...
% % 2nd stage % %
% thrust
Ftb(:,ind_phase23) = throtl(:,ind_phase23).*param.maxT_2nd.*tvc_body(:,ind_phase23);
mpdot = vecnorm(Ftb(:,ind_phase23))./param.Isp2./param.g0;
...
% % % moment computation % % %
Md = zeros(size(Fd)); Mt = zeros(size(Ft));
Macs = zeros(size(Facs)); Mgf = zeros(size(Fgf));
...
for i = 1:dyn_size
    C = EP2C(q(:,i));
    % thrust in iner frame
    Ft(:,i) = C*Ftb(:,i);
    % grid fin control force in iner frame
    Fgf(:,i) = C*param.gf_point*Fgfbn(:,i);
    % acs thrust in iner frame
    Facs(:,i) = C*Facsb(:,i);

    % vehicle dimensions in body frame
    if phase(i) == 0
        rc = param.rc1st(param.m0,param.m.ox2nd,param.m.fuel2nd);
        moic = param.moic1st(param.m0,param.m.ox2nd,param.m.fuel2nd,rc);
        dmoic = param.dmoic1st(param.m0,param.m.ox2nd,param.m.fuel2nd,0,0,0,rc);
    elseif phase(i) == 1
        rc = param.rc1st(m(i),mox(i),mfuel(i));
        moic = param.moic1st(m(i),mox(i),mfuel(i),rc);
        dmoic = param.dmoic1st(m(i),mox(i),mfuel(i),mdot(i),moxdot(i),mfueldot(i),rc);
    elseif phase(i) == 1.5
        rc = param.rc2nd_wplf(param.m02,param.m.ox2nd,param.m.fuel2nd);
        moic = param.moic2nd_wplf(param.m02,param.m.ox2nd,param.m.fuel2nd,rc);
        dmoic = param.dmoic2nd_wplf(param.m02,param.m.ox2nd,param.m.fuel2nd,0,0,0,rc);
    elseif phase(i) == 2
        rc = param.rc2nd_wplf(m(i),mox(i),mfuel(i));
        moic = param.moic2nd_wplf(m(i),mox(i),mfuel(i),rc);
        dmoic = param.dmoic2nd_wplf(m(i),mox(i),mfuel(i),mdot(i),moxdot(i),mfueldot(i),rc);
    elseif phase(i) == 3
        rc = param.rc2nd(m(i),mox(i),mfuel(i));
        moic = param.moic2nd(m(i),mox(i),mfuel(i),rc);
        dmoic = param.dmoic2nd(m(i),mox(i),mfuel(i),mdot(i),moxdot(i),mfueldot(i),rc);
    end
    ...
    Md(:,i) = cross(rcg2cp, C'*Fd(:,i));
    Mt(:,i) = cross(rcg2eng, Ftb(:,i));
    Macs(:,i) = cross(rcg2acs,param.T_acs*param.acs_point)*onoff(:,i);
    Mgf(:,i) = cross(rcg2gf,Fgfbn(:,i)'.*param.gf_point)*ones(4,1);
    % total external moments expressed at center of mass
    Lc = Md(:,i) + Mt(:,i) + Macs(:,i) + Mgf(:,i);
...
x
interactions = [Fg; Fd; Ft; Facs; Fgf; Md; Mt; Macs; Mgf]

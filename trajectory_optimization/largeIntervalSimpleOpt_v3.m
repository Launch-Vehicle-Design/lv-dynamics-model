...
cost_func = @(xopt) -mp_weight*xopt(ntot*N)^2 + ... % minimize fuel used
    vf_weight*(xopt(N)-param.v_final_ref)^2 + ... % final error in orbital velocity
    fpaf_weight*(xopt(2*N)-param.fpa_final_ref)^2 + ... % final error in flight path angle
    altf_weight*(xopt(3*N)-param.alt_final_ref)^2; % final error in orbital altitude

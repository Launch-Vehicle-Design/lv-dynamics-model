...
% 11N+1:11N+3 - the step length for both 1st and 2nd stage w/wo PLF
% 11N+4 - the coast time at stage separation
% 11N+5 - the orbit insertion coast time (optional)
...
weights.duT_weight = 100;

% design optimization variable bounds
throtl_bounds = [0.7 1 0.6 1];
...
N_1st = 100; N_2nd_plf = 20; N_2nd = 180;
param.N_1st = N_1st; param.N_2nd = N_2nd;
param.N_2nd_plf = N_2nd_plf;
N = N_1st+N_2nd+N_2nd_plf; x0 = drop_record.x(:,end);
...
init_x(ntot*N+3) = 2/param.scales.time;
init_x(ntot*N+4) = 0.7/param.scales.time;
if do_orb_ins_c init_x(ntot*N+5) = 0.7/param.scales.time; end
init_x(1:param.nstate*N) = reshape([repmat(x0',[N_1st,1]); ...
    repmat([x0(1:end-1);param.m02+param.mplf]',[N_2nd_plf,1]); ...
    repmat([x0(1:end-1);param.m02]',[N_2nd,1])],[param.nstate*N,1]);
rand_tvc = rand([3,N]);
...
for i = N_1st+2:N_1st+N_2nd_plf+N_2nd
    if i == N_1st+N_2nd_plf+1
        current_x(param.nstate) = current_x(param.nstate)-param.mplf;
    end
    e_v = current_x(4:6)/norm(current_x(4:6));
    e_r = current_x(1:3)/norm(current_x(1:3));
    guess_u = e_r*1+2*e_v;
...
% cost function
cost_func = @(xopt) q_cost(xopt,weights,param);
...
low_bound = [-(param.earthR+2*param.alt_final_ref)*ones([3*N,1]);
    ...
    (param.m02+param.mplf-param.mp2)*ones([N_2nd_plf,1]);
    (param.m02-param.mp2)*ones([N_2nd,1]);
    -1*ones([3*N,1]);
    throtl_bounds(1)*ones([N_1st,1]);
    throtl_bounds(3)*ones([N_2nd_plf+N_2nd,1]);
    h1st_bounds(1);
    h2nd_bounds(1);
    stg_sep_t_bounds(1)];
...
upp_bound = [(param.earthR+2*param.alt_final_ref)*ones([3*N,1]);
    ...
    (param.m02+param.mplf)*ones([N_2nd_plf,1]);
    (param.m02)*ones([N_2nd,1]);
    ones([3*N,1]);
    throtl_bounds(2)*ones([N_1st,1]);
    throtl_bounds(4)*ones([N_2nd_plf+N_2nd,1]);
    h1st_bounds(2);
    h2nd_bounds(2);
    stg_sep_t_bounds(2)];
...
[csol,ceqsol] = nonl_con_sep(x,x0,param);
plotter(x,param);

%% FUNCTION - Cost function
function [f,g] = q_cost(xopt,weights,param)
    N = param.N_1st + param.N_2nd_plf + param.N_2nd;
    % ntot = param.nstate + param.nctrl;
    ...
    f = -weights.mp_weight*xopt(param.nstate*N)/param.mp2; % + ... minimize fuel used
        % weights.duT_weight*sum((control(end,:)-movmean(control(end,:),3)).^2); + ...
...
function [c,ceq] = nonl_con_sep(x,N_1st,N_2nd,x0,param)
    ...
    N_1st = param.N_1st; N_2nd_plf = param.N_2nd_plf; N_2nd = param.N_2nd;
    N = N_1st + N_2nd_plf + N_2nd;
    ntot = param.nstate + param.nctrl;

...
mp_weight = 1e-5; fpaf_weight = 100; vf_weight = 1e-6; altf_weight = 1e-8;
...
for i = 1:N_1st
    init_u = [max(min(-1*(current_x(2)-50/180*pi),tvc_bounds(2)*0.99),tvc_bounds(1)*0.99) init_u(2)];
...
for i = N_1st+2:N_1st+N_2nd
    init_u = [max(min(-1*(current_x(2)-20/180*pi),tvc_bounds(2)*0.99),tvc_bounds(1)*0.99) init_u(2)];
    if init_x(7*N+2) >= 0.5
        dt_2nd = 0.01;

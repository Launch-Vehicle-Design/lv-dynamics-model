function [history, perIndex] = linearOrbitInsertGuidLaw(initState,cond)
    % init history struct
    history.index = 1;
...
function stateNext = update(state, cond, av)
    ...
    xvNext = xv + uv.*cond.dt + 1/2*a.*cond.dt^2;
    uvNext = uv + a.*cond.dt;
...
function [lambdai, lambdavi] = lambdas()
    ...
    Ci = @(tgo,Gi,Gci,v0) Gci/Gi*(cti(tgo, Gi)-1)-v0/sqrt(Gi)*sti(tgo, Gi); Cvi = @(tgo,Gi,Gci,v0) v0*cti(tgo, Gi)-Gci/sqrt(Gi)*sti(tgo, Gi);
    ...
    nui = @(tgo,Gi,Gci,x,u,v0) (d4(tgo,Gi,Gci)*(x-Ci(tgo,Gi,Gci,v0))-d2(tgo,Gi,Gci)*(u-Cvi(tgo,Gi,Gci,v0)))...
        /(d1(tgo,Gi,Gci)*d4(tgo,Gi,Gci)-d2(tgo,Gi,Gci)*d3(tgo,Gi,Gci));
    nuvi = @(tgo,Gi,Gci,x,u,v0) (-d3(tgo,Gi,Gci)*(x-Ci(tgo,Gi,Gci,v0))+d1(tgo,Gi,Gci)*(u-Cvi(tgo,Gi,Gci,v0)))...
        /(d1(tgo,Gi,Gci)*d4(tgo,Gi,Gci)-d2(tgo,Gi,Gci)*d3(tgo,Gi,Gci));
    lambdai = @(tgo,Gi,Gci,x,u,v0) 1/2*(nui(tgo,Gi,Gci,x,u,v0)-sqrt(Gi)*nuvi(tgo,Gi,Gci,x,u,v0))*exp(-sqrt(Gi)*tgo)...
        + 1/2*(nui(tgo,Gi,Gci,x,u,v0)+sqrt(Gi)*nuvi(tgo,Gi,Gci,x,u,v0))*exp(sqrt(Gi)*tgo);
    lambdavi = @(tgo,Gi,Gci,x,u,v0) -1/2/sqrt(Gi)*(nui(tgo,Gi,Gci,x,u,v0)-sqrt(Gi)*nuvi(tgo,Gi,Gci,x,u,v0))*exp(-sqrt(Gi)*tgo)...
        + 1/2/sqrt(Gi)*(nui(tgo,Gi,Gci,x,u,v0)+sqrt(Gi)*nuvi(tgo,Gi,Gci,x,u,v0))*exp(sqrt(Gi)*tgo);
end
...
function [t, control] = tgoSolve(state, cond, lambdai, lambdavi)
    ...
    for i = 1:3
        ...
        sum = @(tgo) sum(tgo) - lambdavi(tgo,Gi,Gci,x,u,cond.v0(i))^2/2+lambdai(tgo,Gi,Gci,x,u,cond.v0(i))*u...
            +lambdavi(tgo,Gi,Gci,x,u,cond.v0(i))*(Gci+Gi*x);
        switch i
            case 1
                a = @(tgo) a(tgo) + [-1*lambdavi(tgo,Gi,Gci,x,u,cond.v0(i)); 0; 0];
            case 2
                a = @(tgo) a(tgo) + [0; -1*lambdavi(tgo,Gi,Gci,x,u,cond.v0(i)); 0];
            case 3
                a = @(tgo) a(tgo) + [0; 0; -1*lambdavi(tgo,Gi,Gci,x,u,cond.v0(i))];
        end
    end

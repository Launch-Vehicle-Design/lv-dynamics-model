...
init_x(7*N+2) = 30; 
init_x(7*N+3) = 5;
% init_x(1:5*N) = reshape(repmat(x0,[1,N])',[5*N,1]);
...
current_x = x0;
for i = 1:N_1st
    init_u = [(tvc_bounds(2)-tvc_bounds(1))*rand()+tvc_bounds(1) ...
        (throtl_bounds(2)-throtl_bounds(1))*rand()+throtl_bounds(1)];
    dynfunc_1st = @(t,x,u) rocket(t,x,u,param,1);
    if init_x(7*N+1) >= 0.5
        dt_1st = 0.01;
        u = repmat(init_u,[ceil(init_x(7*N+1)/dt_1st),1]);
        record = rk4sim(dynfunc_1st,current_x',dt_1st,init_x(7*N+1),u);
    ...
    init_x(param.init_ind_ptr(6:7)+i) = init_u;
end
...
for i = N_1st+2:N_1st+N_2nd
    ...
    if init_x(7*N+2) >= 0.5
        dt_2nd = 0.01;
        u = repmat(init_u,[ceil(init_x(7*N+2)/dt_2nd),1]);
        record = rk4sim(dynfunc_2nd,current_x',dt_2nd,init_x(7*N+2),u);
        current_x = record.x(end,:)';
    else
        u = init_u;
...
cost_func = @(xopt) -mp_weight*xopt(5*N)^2 + ... % minimize fuel used
    ...
    % abs(xopt(7*N+1))*(dtvc_weight_1st*sum(diff(xopt(5*N+1:5*N+1+N_1st)).^2) + dthrotl_weight_1st*sum(diff(xopt(6*N+1:6*N+N_1st)).^2)) + ...
    % abs(xopt(7*N+2))*(dtvc_weight_2nd*sum(diff(xopt(5*N+1+N_1st:6*N)).^2) + dthrotl_weight_2nd*sum(diff(xopt(6*N+1+N_1st:7*N)).^2)); % minimize control frequency
...
function [c,ceq] = nonl_con_sep(x,N_1st,N_2nd,x0,param)
    ...
    for i = 1:N_1st
        ...
        if h_1st >= 0.5
            ...
            ceq(5*(i-1)+1:5*i) = next_x - step_record.x(end,:)';
        else
    ...
    ceq(5*N+1) = x(init_ind_ptr(6)+N_1st+1,:);
    ...
    ceq(5*N+2) = x(N)-param.v_final_ref; % final error in orbital velocity
    ceq(5*N+3) = x(2*N)-param.fpa_final_ref; % final error in flight path angle
    ceq(5*N+4) = x(3*N)-param.alt_final_ref; % final error in orbital altitude

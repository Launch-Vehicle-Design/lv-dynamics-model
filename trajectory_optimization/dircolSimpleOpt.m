...
param.scale.alti = 500e3;
param.scale.fpa = pi/2;
...
throtl_bounds = [0.4 1 0.8 1];
h1st_bounds = [0.01 5];
h2nd_bounds = [0.01 5];
stg_sep_t_bounds = [0.01 60];
...
current_x = x0;
for i = 1:N_1st
    init_u = [max(min(-1*(current_x(2)-60/180*pi),tvc_bounds(2)*0.99),tvc_bounds(1)*0.99) 0.7];
...
file_name = "dircol_sqp_intermed3.mat";
if load_prev && exist(file_name,"file")
    ...
    init_x(ntot*N+3) = log_x(log_ntot*log_N+3);
end
param.axes = plotter(init_x,drop_time,N_1st,N_2nd,param);
...
options = optimoptions('fmincon','Display','iter','Algorithm','interior-point',...
    'MaxFunctionEvaluations',1e8,'MaxIterations',1e7,'ConstraintTolerance',1e-4);
...
function axes = plotter(x,drop_time,N_1st,N_2nd,param)
    N = N_1st + N_2nd;
    ntot = param.nstate + param.nctrl;
    ...
    axes(1) = subplot(2,3,1); plot([t_1st t_2nd], x(param.init_ind_ptr(1)+(1:N)),"k","LineWidth",1.2); grid on
    xlabel("Time since Release (s)"); ylabel("Vehicle Velocity (m/s)");
    axes(2) = subplot(2,3,2); plot([t_1st t_2nd], x(param.init_ind_ptr(2)+(1:N))*180/pi,"k","LineWidth",1.2); grid on
    xlabel("Time since Release (s)"); ylabel("Pitch angle (deg)");
    axes(3) = subplot(2,3,3); plot([t_1st t_2nd], x(param.init_ind_ptr(3)+(1:N))/1000,"k","LineWidth",1.2); grid on
    xlabel("Time since Release (s)"); ylabel("Altitude (km)");
    axes(4) = subplot(2,3,4); plot([t_1st t_2nd], x(param.init_ind_ptr(4)+(1:N)),"k","LineWidth",1.2); grid on
    xlabel("Time since Release (s)"); ylabel("Vehicle Mass (kg)");
    axes(5) = subplot(2,3,5); plot([t_1st t_2nd], x(param.init_ind_ptr(5)+(1:N))*180/pi,"k","LineWidth",1.2); grid on
    xlabel("Time since Release (s)"); ylabel("Gimbal Angle (deg)");
    axes(6) = subplot(2,3,6); plot([t_1st t_2nd], x(param.init_ind_ptr(6)+(1:N))*100,"k","LineWidth",1.2); grid on
...
function [f,g] = q_cost(xopt,N_1st,N_2nd,weights,param)
    ...
    f = -weights.mp_weight*(xopt(param.nstate*N)/param.mp2)^2 + ... % minimize fuel used
        weights.dutvc_weight*sum((diff([xopt(4*N+1:4*N+N_1st)*xopt(ntot*N+1);xopt(4*N+1+N_1st:5*N)*xopt(ntot*N+2)])/param.scale.fpa).^2) + ...
        weights.duT_weight*sum((xopt(5*N+1:6*N)-mean(xopt(5*N+1:6*N))).^2) + ...
        weights.altf_weight*sum((([xopt(2*N+1:2*N+N_1st)*xopt(ntot*N+1);xopt(2*N+1+N_1st:3*N)*xopt(ntot*N+2)]-param.alt_final_ref)/param.scale.alti).^2) + ...
        weights.vf_weight*sum((([xopt(1:N_1st)*xopt(ntot*N+1);xopt(1+N_1st:N)*xopt(ntot*N+2)]-param.v_final_ref)/param.scale.velo).^2);
end
...
function [c,ceq] = nonl_con_sep(x,N_1st,N_2nd,x0,param)
    ...
    % max Q restriction
    q = 1/2*param.rho(x(2*N+1:3*N)).*x(1:N).^2;
    % c(3) = max(q);

    % visualization
    if mod(func_count,500) == 0
        param.axes = plotter(x,5,N_1st,N_2nd,param); drawnow
    end
...
%% FUNCTION - Gradient of dynamics
function [gx,gu] = grad_rocket(t,x,u,param,stage)
    v = x(1); fpa = x(2); h = x(3); m = x(4);
    tvc = u(1); throtl = u(2);
    h0 = 7640; g0 = 9.80665; rho0 = 1.225;
    % fit of differential coefficien of drag
    
    gx = [exp(-h/h0)*param.S.*v*rho0]
end
